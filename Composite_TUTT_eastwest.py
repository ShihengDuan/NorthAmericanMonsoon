...
east_anomaly_composite = []
west_anomaly_composite = []
for tutt in tqdm(NAM_tutt[:]):
    ...
    for i, time in enumerate(tutt_time):
        ...
        if init_lon<end_lon: # eastward
            east_anomaly_composite.append(precip_anomaly.data[:80, :80].reshape(1, 80, 80)) # time, lat, lon
        else:
            west_anomaly_composite.append(precip_anomaly.data[:80, :80].reshape(1, 80, 80)) # time, lat, lon

east_anomaly_composite = np.concatenate(east_anomaly_composite)
west_anomaly_composite = np.concatenate(west_anomaly_composite)
print(west_anomaly_composite.shape)
np.save('west_TUTT_anomaly_precip_20degree', west_anomaly_composite)
np.save('east_TUTT_anomaly_precip_20degree', east_anomaly_composite)
